...
popularMoviesDf = mostPopularMoviesDf.join(movieDf, mostPopularMoviesDf.movieId == movieDf.movieId)\
                                     ...
                                     .withColumn("round", round("avg_rating", 2))\
                                     .sort(desc("total_ratings"))
...
# rating analytics data without movies
# Displays parsed plan - written as is
# bottom to top 
...
print(popularMoviesDf.rdd.getNumPartitions())
# write statements work on exeuctors where each partitions information stored
# executor the one write the results to file system
popularMoviesDf.write.mode("overwrite")\
                .option("header", True)\
                .csv("/FileStore/tables/tmp2/popular-movies-csv-files")
...
# coalesce(1) to reduce partitions to 1
popularMoviesDf.coalesce(1).write.mode("overwrite")\
                .option("header", True)\
                .csv("/FileStore/tables/tmp2/popular-movies-csv")

# COMMAND ----------


# inferSchema will scan csvs and define data types for  your schema
popularMovies = spark.read.format("csv")\
                .option("header", True)\
                .option("inferSchema", True)\
                .load("/FileStore/tables/tmp2/popular-movies-csv-files")
...
popularMoviesDf.coalesce(1).write.mode("overwrite")\
                .parquet("/FileStore/tables/tmp2/popular-movies-parquet")
...
# coalesce(1) to reduce partitions
popularMoviesDf.coalesce(1).write.mode("overwrite")\
                .json("/FileStore/tables/tmp2/popular-movies-json")
...
# coalesce(1) to reduce partitions
popularMoviesDf.coalesce(1).write.mode("overwrite")\
                .orc("/FileStore/tables/tmp2/popular-movies-orc")

# COMMAND ----------

popularMovies = spark.read.format("json")\
                .option("inferSchema", True)\
                .load("/FileStore/tables/tmp2/popular-movies-json")
